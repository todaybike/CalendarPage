...
class HCalendarConst {
	/** 일주일의 최대 날수				 	*/
	static readonly MAX_DAYOFWEEK : number		= 7;
	/** 1년의 최대 월 수					*/
	static readonly MAX_MONTH : number			= 12;
	/** 달력 페이지에 보이는 최대 날짜 수		*/
	static readonly MAX_PAGE_DAY : number		= 42;
	/** 달력 페이지에 보이는 최대 주 수			*/
	static readonly MAX_PAGE_WEEK : number		= 6;

	/** 날짜 구분 : 평일 1					*/
	static readonly KIND_DATE_NORMAL : number	= 1;
	/** 날짜 구분 : 일요일 0				*/
	static readonly KIND_DATE_SUN : number 		= 0;
	/** 날짜 구분 : 토요일 6				*/
	static readonly KIND_DATE_SAT : number 		= 6;
	/** 날짜 구분 : 기념일 7				*/
	static readonly KIND_DATE_EVENT : number	= 7;
	/** 날짜 구분 : 이번달 아님 8			 */
	static readonly KIND_DATE_NO_MONTH : number	= 8;	// 이번달 아님
}
...
class HDay {
	 ...
	 * @param d 
	 */
	constuctor( y:number, m:number, d:number ) : HDay {
		this._setKind( y, m, d );
	...
	protected _setKind( y:number, m:number, d:number ) : number {
		...
		let today = moment( { year:y, month:m, date:d } );
...
class HWeek {
	...
	public set( y:number, m:number, d:number ) : number {
		...
		let firstDay		= moment( { year:y, month:m, date:d} );
		let lastDay			= moment( { year:y, month:m, date:d} );
	 ...
	 * @param d 일
	 */
	public getWeek( y:number, m:number, d:number ) : Array<HDay> {
		this.set( y,m,d );
		return this._days; 
	}
...
class HCalendarPage {
	 ...
	 */
	public next() {
		let nextDate = new Date( this._today.year, this._today.month + 1, 1 );	
	 ...
	 */
	public prev() {
		let prevDate = new Date( this._today.year, this._today.month - 1, 1 );	
	...
	public set( y:number, m:number, d:number=1 ) : number {
		...
		let firstDate		= new Date(y, m, 1);		// 해당 월의 첫번째 날을 구한다.
		let lastDate		= new Date(y, m+1, 0);		// 해당 월의 마지막 날을 구한다.

		let firstDayOfWeek	= firstDate.getDay();		// 첫번째 날의 주중 인덱스를 구한다.
		let lastDayOfWeek	= lastDate.getDay();		// 마지막 날의 인덱스를 구한다.
		
		// 첫째 날이 속한 주간의 첫번째 날을 구한다.
		// 달력 페이지에 보이는 첫번째 날이다. 해당 월의 첫번째와는 구분되어야 한다.
		let viewFirstDate	= new Date( y, m, -(firstDayOfWeek-1) );
		...
		let viewLastDate	= new Date( y, m+1, (6 - lastDayOfWeek) )	
		...
		let first			= moment(viewFirstDate);
		let last			= moment(viewLastDate);	
